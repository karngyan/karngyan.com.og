...
<div class="max-w-7xl pb-10 border-r border-l border-dashed border-gray-700 mx-auto">
  <div data-aos="fade-up" class="relative max-w-7xl border-r border-l border-dashed border-gray-700 py-6 mx-auto">
  ...
  <div data-aos="zoom-in" class="max-w-4xl mx-4 sm:mx-auto bg-gray-900 shadow rounded-md overflow-hidden sm:rounded-lg">
    ...
    <div>
      <dl>
        <div v-for="(item, index) in $store.state.uses.meta" :class="index%2 === 0 ? 'bg-gray-800' : 'bg-gray-900'" class="px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
...
export default {
  ...
  mounted() {
    fetch(`https://codestats.net/api/users/${this.$store.state.social.codestats}`)
      .then(resp => resp.json())
      .then(data => {
