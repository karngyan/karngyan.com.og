        ...
        <div class="space-y-9">
          ...
          <div class="px-4 sm:px-0 lg:px-4 pb-4 bg-gray-900 rounded-lg">
            ...
            <div class="flex flex-items-center justify-center  rounded-full">
              <img class="w-12 h-12 border-2 border-indigo-600 hover:border-hot-pink -mt-5 bg-gray-900 rounded-full" :src="article.author.image" alt="Author">
            </div>
            </a>
            <header class="py-4">
                ...
                <dl class="">
                  <div><dt class="sr-only">Published on</dt>
                    <dd class="text-xs font-medium text-gray-500">
                      <a target="_blank" rel="noreferrer" :href="`https://twitter.com/${article.author.twitter}`"><span class="text-indigo-600 hover:text-indigo-300">{{ article.author.name }}</span></a>
...
<script>
export default {
  async asyncData({ $content, params, route, store }) {
