...
<script>
export default {
  async asyncData({ $content, params, store, route }) {
    const project = await $content('projects', params.slug).fetch()
    project.twitterShareUrl = `https://twitter.com/intent/tweet?text=${project.title} by @${project.twitter}&url=https://${store.state.domain}${route.fullPath}`
    return {
      project,
    }
  },
  head() {
    return {
      title: this.project.title + ' | projects -- Karn | Friendly Neighborhood Developer',
      meta: [
